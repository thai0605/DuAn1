...
class AdminGiaodienController
{
   ...
   public function postaddBanner()
   {
      if ($_SERVER['REQUEST_METHOD'] == 'POST') {
         ...
         if (isset($Image) && $Image['error'] === 0) {
            ...
            $errors['Image'] = 'Bạn cần chọn một ảnh để tải lên';
         }
         ...
         if (
            $this->modelgiaodien->insertBanner(
               $Title,
               $Description,
   ...
   public function deleteBanner()
   {
      ...
      if (isset($_GET['ID'])) {
         ...
         $banner = $this->modelgiaodien->getAllBanners();
   ...
   public function postEditBanner()
   {
      ...
      if ($_SERVER['REQUEST_METHOD'] == 'POST') {
         ...
         if (empty($file_thumb)) {
            $file_thumb = $banner['Image'];
         }

         // Cập nhật banner nếu không có lỗi
         if (
            $this->modelgiaodien->updateBanners(
               $ID,
               $Title,
               $Description,
